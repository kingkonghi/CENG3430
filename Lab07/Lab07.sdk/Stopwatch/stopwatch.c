...
int main(void){
	...
	while(1)
	{
		...
		if (btn_rise & BTN_C) {b=(b==1?0:1);stopped=(stopped==1?0:1);}
		if (btn_rise & BTN_L) {b=(b==2?0:2);stopped=(stopped==1?0:1);}
		...
		{	u32 time_elapsed=0;
			if(b==1) time_elapsed = (timer_in - timer_zero) / 1000; // seconds
			if(b==2) time_elapsed = (timer_in - timer_zero) / 500;
